...
#define GRA_AND_AFCH_AD9959_MOD //Добавлена функция SetReferenceFreq
...
public:
  ...
  AD9959()  : core_clock(0)  , last_channels(0xF0)
  {
  ...
  //void setClock(int mult = 12, int32_t calibration = 0) // Mult must be 0 or in
  void setClock(int mult = 12, uint32_t refFreq = reference_freq, int32_t calibration = 0) // Mult must be 0 or in range 4..20
  {
    if (mult < 4 || mult > 20)
      mult = 1;                         // Multiplier is disabled.
	//core_clock = reference_freq * (1000000000ULL+calibration) / 1000000000ULL * mult;
    core_clock = refFreq * (1000000000ULL+calibration) / 1000000000ULL * mult;
#if defined(DDS_MAX_PRECISION)
...
protected:
  ...
  uint32_t write(uint8_t reg, uint32_t value)
  {
    ...
    uint32_t    rval = 0;
    int         len = (reg&0x7F) < sizeof(register_length) ? register_length[reg&0x7F] : 4;
    spiBegin();
    SPI.transfer(reg);
    while (len-- > 0)
