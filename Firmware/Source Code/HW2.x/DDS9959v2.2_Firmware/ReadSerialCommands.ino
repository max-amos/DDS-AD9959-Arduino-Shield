...
const char HELP_STRING [] PROGMEM = "C — Set the current output Channel: (0 — 3)\n"
          "F — Sets Frequency in Hz (100000 — 225000000), supports scientific notation (e.g. F1e6, F1.5e8)\n"
    ...
    for (int i=0; i < commandsCounter; i++)
    {
      ...
      switch (command)
      {
        ...
        case 'F': //RF Frequency
          ...
          {
            // Re-parse with strtod() to support scientific notation (e.g. F1e6, F1.5e8)
            value = (int32_t)strtod(data[i] + 1, NULL);
          }
